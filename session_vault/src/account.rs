...
use crate::utils::*;
use crate::*;
use near_sdk::{env, is_promise_success, log, near_bindgen, AccountId, Balance, PromiseOrValue};
...
impl Contract {
    pub fn internal_deposit_to_account(&mut self, account_id: &AccountId, amount: Balance) {
        ...
        assert!(
            account.locking_amount() == 0 && account.last_claim_session != account.session_num,
            "ERR_ALREADY_DEPOSITED"
        );
        assert!(
            account.session_num as Balance * account.release_per_session == amount,
...
impl Contract {
    pub fn claim(&mut self, account_id: Option<ValidAccountId>) -> PromiseOrValue<bool> {
        ...
        assert!(amount <= account.locking_amount(), "ERR_NOT_ENOUGH_BALANCE");
...
impl FungibleTokenReceiver for Contract {
    ...
    fn ft_on_transfer(
        ...
        if msg.is_empty() {
            ...
            let contract_id: String = env::current_account_id();
            if msg.eq_ignore_ascii_case(contract_id.as_str()) {
                env::panic(b"ERR_RECIPIENT_CANNOT_BE_SESSION_VAULT_CONTRACT");
            }
            self.internal_deposit_to_account(&msg, amount);
        }

        let sender: AccountId = sender_id.into();
        log!("{} deposit token to {}, amount: {}", sender, msg, amount);
